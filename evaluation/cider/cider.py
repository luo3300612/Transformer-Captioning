...
class Cider:
    ...
    def __init__(self, gts=None, n=4, sigma=6.0, get_cache=False):
        # set origin_cider to sum over 1 to 4-grams
        self._n = n
        ...
        if gts is not None:
            ...
            self.gts_cache = tmp_cider.gts_cache

    # def real_init(self, gts=None, n=4, sigma=6.0):
    #     # set origin_cider to sum over 1 to 4-grams
    #     self._n = n
    #     # set the standard deviation parameter for gaussian penalty
    #     self._sigma = sigma
    ...
    def compute_score(self, gts, res):
        ...
        :return: origin_cider (float) : computed CIDEr score for the corpus
        """
        assert (gts.keys() == res.keys())
        cider_scorer = CiderScorer(gts, test=res, n=self._n, sigma=self._sigma, doc_frequency=self.doc_frequency,
                                   ref_len=self.ref_len, cache=self.gts_cache)
